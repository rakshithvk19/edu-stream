...
export async function GET(request: NextRequest) {
  try {
    ...
    const limit = Math.min(50, Math.max(1, parseInt(searchParams.get('limit') || '12', 10))); // Max 50 videos per page
    const searchQuery = searchParams.get('search') || undefined;

    // Get ready videos from database
    const { videos, total } = await videoRepository.getReadyVideos(page, limit, searchQuery);
