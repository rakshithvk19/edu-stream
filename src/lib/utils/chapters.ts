import { MAX_CHAPTERS, MIN_CHAPTER_DURATION, MAX_CHAPTER_TITLE_LENGTH } from '@/lib/constants/upload';
...
export function parseChaptersFromText(chaptersText: string): {
  ...
  lines.forEach((line, index) => {
    ...
    try {
      ...
      try {
        ...
        errors.push({
          line: lineNumber,
          error: timestampError instanceof Error ? timestampError.message : 'Invalid timestamp',
          rawLine: line,
        });
        return;
      }
  ...
  chapters.forEach((chapter, index) => {
    ...
    if (usedTimestamps.has(chapter.start_seconds)) {
      const duplicateLine = lines.findIndex(line => line.includes(chapter.timestamp)) + 1;
      errors.push({
        line: duplicateLine,
    ...
    if (index > 0) {
      ...
      if (duration < MIN_CHAPTER_DURATION) {
        const chapterLine = lines.findIndex(line => line.includes(chapter.timestamp)) + 1;
        errors.push({
          line: chapterLine,
  ...
  if (chapters.length > 0 && chapters[0].start_seconds !== 0) {
    const firstLine = lines.findIndex(line => line.includes(chapters[0].timestamp)) + 1;
    errors.push({
      line: firstLine,
  ...
  // Ensure minimum 3 chapters if any chapters provided
  if (chapters.length > 0 && chapters.length < 3) {
    errors.push({
      line: chapters.length,
      error: 'At least 3 chapters required if using chapters',
      rawLine: '',
    });
  }
 ...
 */
export function calculateChapterEndTimes(chapters: Chapter[], videoDurationSec?: number): (Chapter & { end_seconds?: number })[] {
  const sortedChapters = [...chapters].sort((a, b) => a.start_seconds - b.start_seconds);
 ...
 */
export function validateChaptersAgainstDuration(chapters: Chapter[], videoDurationSec: number): ChapterValidationError[] {
  const errors: ChapterValidationError[] = [];
